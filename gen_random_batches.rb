...
file['products'].each do |name, data|
	...
	gen.rng((25..55).sort_by{rand}[0]).map(&:to_i).each do |qtd|
		...
		puts %(<batches id="#{id}" product_id="#{data['id']}" user_id="#{user_id}" created_at="#{base_date.strftime time_format}" updated_at="#{base_date.strftime time_format}" manufactured_at="#{manufactured_at.strftime time_format}" expires_at="#{expires_at.strftime time_format}" identifier="#{identifier}" quantity="#{qtd}" />)
		seq += 1
		id += 1
	end
end
